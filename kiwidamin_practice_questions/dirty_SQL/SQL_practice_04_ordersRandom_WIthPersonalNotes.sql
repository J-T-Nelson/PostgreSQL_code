...
SELECT co.customer_id, c.name, c.address, c.state, COUNT(co.order_id) as num_orders
...
SELECT p.price, co.order_id, op.product_id
FROM customer_order co
JOIN order_product op ON op.order_id = co.order_id
JOIN product p ON p.product_id = op.product_id
WHERE co.customer_id = 1026

-- With some help from ChatGPT .. I was quite close, but the wording of the question made it confusing to start. 
--	Didn't know I could use distinct this way either
-- THE ANSWER:
SELECT 
    co.order_id, 
    SUM(p.price) as total_spent, 
    COUNT(DISTINCT op.product_id) as total_distinct_products
FROM 
    customer_order co
JOIN 
    order_product op ON op.order_id = co.order_id
JOIN 
    product p ON p.product_id = op.product_id
WHERE 
    co.customer_id = 1026
GROUP BY 
    co.order_id; -- this answer actually has some issues... the total_spent col is incorrect. others are good though. needed to multiply by value in qty

-- what it looks like to remove the DISTINCT clause .. actually the same exact answer. 
-- Meaning no body bought more of 1 of the same product in these orders. 
SELECT 
    co.order_id, 
    SUM(p.price) as total_spent, 
...
DROP VIEW customer_orders_products; -- dropping previously incorrect view
-- inspect VIEW: 
